...
def run_sanity_check(test_dir):
    ...
    test_functions_for_get = list(filter(lambda x: inspect.getsource(getattr(
        module, x)).find(".get(") != -1, test_function_names))
    test_functions_for_post = list(filter(lambda x: inspect.getsource(getattr(
        module, x)).find(".post(") != -1, test_function_names))
    ...
    if not test_functions_for_post:
        ...
        if len(test_functions_for_post) == 1:
            ...
            WARNING_COUNT += 1
            print(FAIL_COLOR+"Only one test case was detected for the POST() "
                  "method.")
        ...
        _con3 = COUNT_POST_METHOD_TEST_FOR_INFERENCE_RESULT < 2
        if len(test_functions_for_post) >= 2 and _con3:
            print(FAIL_COLOR+f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
